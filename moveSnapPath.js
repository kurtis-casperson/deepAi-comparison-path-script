...
const bootedDeviceRow =
  devices.split('\n').find((row) => row.includes('(Booted)')) ?? ''
const deviceUUID = bootedDeviceRow.substr(bootedDeviceRow.length - 47, 36)

const readCleanDirSync = (path) =>
  fs.readdirSync(path).filter((fileName) => !fileName.includes('DS_Store'))
...
const applicationIdPath = path.join(
  `${homedir}`,
...
for (let i = 0; i < applicationIdFolders.length; i++) {
  ...
  try {
    const { mtimeMs: date } = fs.statSync(
      path.join(applicationIdPath, currentFolder)
    )
...
for (let i = 0; i < newSnapShotsTrimmed.length; i++) {
  if (!existingSnapShotsTrimmed.includes(newSnapShotsTrimmed[i])) {
    fs.copyFileSync(
      `${documentFolder}/${newSnapShots[i]}`,
      `${snapShotFolder}/${newSnapShots[i]}`
    )
  }
}
  ...
  for (let i = 0; i < newSnapShots.length; i++) {
    ...
    try {
      ...
      const snapShotResult =
        snapShotDifference <= 2
          ? `Same Snapshot `
          : `Different Snapshot Distance `
      ...
      report.push({
        ...snapShotReport,
        snapShotError: `Failed To Respond for snapshot ${newSnapShots[i]}`,
      })
    }
  }
  return report
